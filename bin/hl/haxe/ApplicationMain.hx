...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("TestMain", create);

		#if (js && html5)
		#if (munit || (utest && openfl_enable_utest_legacy_mode))
		lime.system.System.embed("TestMain", null, 800, 600);
		#end
		#else
		create(null);
		#end
	...
	public static function create(config):Void
	{
		...
		app.meta["build"] = "103";
		app.meta["company"] = "";
		app.meta["file"] = "TestMain";
		app.meta["name"] = "FlixelUnitTests";
		...
		#elseif air
		app.window.title = "FlixelUnitTests";
		#else
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("TestMain")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess TestMain.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"TestMain\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"TestMain\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends TestMain {}
#else
class DocumentClass
{
