...
class SpriteGroupCameraTestState extends flixel.FlxState
{
    override function create()
    {
        ...
        final uiGroup = new FlxSpriteGroup();
        add(uiGroup);
        
        final healthBars = new FlxTypedSpriteGroup<Health>();
...
class Health extends FlxSpriteGroup
{
    ...
    override function draw()
    {
        ...
        function getCam(obj:FlxBasic)
            return obj._cameras != null && obj._cameras.length > 0
                ? Std.string(obj._cameras[0] == FlxG.camera ? "main" : "ui")
                : "null"
                ;
        
        FlxG.watch.addQuick('$ID', getCam(this));
        if (group.members.length > 0)
            FlxG.watch.addQuick('$ID-bar', getCam(group.members[0]));
    }
}
